...
private:
	...
	template<typename uint> uint read_int();
	void read_entry(entry& e, int index);
	int find_key(const key64& key);

public:
	std::string std_name{ "Monolith.bin" };
	bool hit{ true };

	bool open(std::string file);

	// searching the book for a position-match

	move get_move(const board& pos);
};

namespace polyglot
{
	key64 position_key(const board &pos);

	// random hash numbers generated by Fabien Letouzey to index book entries

	constexpr std::array<key64, 768> key_pc
	{ {
	...
	constexpr key64 key_cl{ 0xF8D626AAAF278509ULL };
}