...
private:
	...
	static rand_32 rand_gen;

	// locating a matching book entry

	template<typename uint> static uint read_int();
	static void read_entry(book_entry &entry, int idx);
	static int find_key(uint64 &key);

	static uint32 encode_move(uint16 move16, const board &pos);
...
public:
	...
	// searching the book for a match

	static uint32 get_move(board &pos, bool best_line);
};

// random numbers generated by Fabien Letouzey

namespace polyglot
{
	uint64 to_key(const board &pos);

	const uint64 rand_key[781]
	{
