...
void pos::new_move(uint16 move)
{
	...
	if (victim != NONE)
	{
		...
		piece12 = hashing::piece_12[victim];
	...
	if (piece == PAWNS && abs(sq1 - sq2) == 16)
	{
		...
		auto file{ sq1_64 & 7 };
		if (pieces[PAWNS] & side[turn ^ 1] & hashing::ep_flank[turn][file])
			key ^= hashing::random64[hashing::offset.ep + 7 - file];
	}
	...
	piece12 = hashing::piece_12[piece];
	...
	if (flag >= 8)
	{
		...
		else
		{
			...
			piece12 = hashing::piece_12[promo_p];
			int sq_new{ mirror(sq2) };
			key ^= hashing::random64[(BLACK - turn) * 64 + sq_new];
			key ^= hashing::random64[(piece12 - turn) * 64 + sq_new];
	...
	while (focus < fen.size() && fen[focus] != ' ')
	{
		...
		else
		{
			for (int piece{ PAWNS }; piece <= KINGS; ++piece)
				...
				{
					if (fen[focus] == p_char[color][piece])
					{
